<?php
/**
 * tools/fix-composer.php (v6)
 * - Works in given working directory (so run it in detected root)
 * - Remove direct illuminate/* from require + require-dev
 * - Remove fideloper/proxy (legacy)
...
if (isset($json['conflict']) && is_array($json['conflict'])) {
    ...
    if (empty($json['conflict'])) { unset($json['conflict']); $changed = true; }
}
